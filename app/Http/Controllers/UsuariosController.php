...
class UsuariosController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($tam > 1){
            return redirect('/usuarios_agregar')->with('flash_message_error','Error este correo ya fue registrado');
        }else{
            if(strlen($contra) < 6 ){
    ...
    {       
        ...
        if($contra==null){
            ...
            if(strlen($contra) < 6 ){
                return redirect($url)->with('flash_message_error','La contraseña debe tener almenos 6 caracteres');
            }else{
                $usuarios = \App\User::find($id); 
                $usuarios->name=$request->input('name');
                $usuarios->email=$request->input('email');
                ...
                $usuarios ->rol_id = $request->get('rol_id');
                $usuarios->save();
                return redirect('/usuarios')->with('success', 'Información actualizada exitosamente');  
            }
        }
    } 
